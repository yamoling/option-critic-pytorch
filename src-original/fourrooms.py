...
"""
    ...
    def reset(self):
        state = int(np.random.choice(self.init_states))
    ...
    def switch_goal(self):
        prev_goal = self.goal
        self.goal = int(np.random.choice(self.init_states))
    ...
    def step(self, action):
        ...
        if not self.occupancy[nextcell]:
            if np.random.random() < 1 / 3.0:
                empty_cells = self.empty_around(self.currentcell)
                self.currentcell = empty_cells[int(np.random.randint(len(empty_cells)))]
            else:
