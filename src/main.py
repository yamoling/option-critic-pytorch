...
def run(args):
    ...
    while steps < args.max_steps_total:
        ...
        if args.switch_goal and logger.n_eps == 1000:
            torch.save({"model_params": option_critic.state_dict(), "goal_state": env.goal}, f"models/option_critic_seed={args.seed}_1k")
            env.switch_goal()
            print(f"New goal {env.goal}")

        if args.switch_goal and logger.n_eps > 2000:
            torch.save({"model_params": option_critic.state_dict(), "goal_state": env.goal}, f"models/option_critic_seed={args.seed}_2k")
            break
