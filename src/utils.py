...
class LazyFrames(object):
    ...
    def __array__(self, dtype=None):
        return np.concatenate(self._frames, axis=0, dtype=dtype)
...
def make_env(env_name):
    ...
    is_atari = hasattr(gym.envs, "atari") and isinstance(env.unwrapped, gym.envs.atari.atari_env.AtariEnv)  # type: ignore
    if is_atari:
        env = AtariPreprocessing(env, grayscale_obs=True, scale_obs=True, terminal_on_life_loss=True)
        env = TransformReward(env, lambda r: np.clip(float(r), -1, 1))
